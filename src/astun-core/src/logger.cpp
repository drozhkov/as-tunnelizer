...
std::mutex as::loggerMutex;
std::ostream * as::loggerStream = &std::cout;


void as::loggerInit( const std::string_view path )
{
	loggerStream = new std::ofstream( path.data(), std::ios::out | std::ios::app );
}
